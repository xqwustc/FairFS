...
class DCN(BaseModel):
    ...
    def forward_with_fmcr(self, inputs, inter_type = 'zero'):
        X = self.get_inputs(inputs)
        ...
        interp_n = getattr(self, 'interpolate_n', 0) or 0
        if interp_n > 0:
            self.feature_emb_delta_step = (self.feature_emb - self.feature_emb_mean) / interp_n
    ...
    def evaluate_with_fmcr(self, data_generator, baseline = 'zero',metrics=None):
        y_pred = []
        y_true = []
        group_id = []
        ...
        for batch_data in data_generator:
            return_dict = self.forward_with_fmcr(batch_data, inter_type=baseline)

            y_true_fmcr = self.get_labels(batch_data)
            y_true_fmcr = y_true_fmcr.repeat(interp_n + 1, 1)
            ...
            fmcr_field_delta = [i.repeat(interp_n + 1, 1) for i in
    ...
    def evaluate_with_fmcr_native(self, data_generator, metrics=None, **kwargs):
        self.eval()
        feature_importance_result, native_log_loss = self.evaluate_with_fmcr(
            data_generator, baseline=kwargs.get('baseline', 'zero'), metrics=None)
    ...
    def fit_for_embig(self, data_generator, epochs=1, validation_data=None,
        ...
        # [cite_start]'normk' corresponds to the regularization coefficient lambda (phi) in the paper [cite: 38, 761]
        ...
        for epoch in range(epochs):
            ...
            for batch_index, batch_data in enumerate(batch_iterator):
                ...
                # [cite_start]and the smoothed baseline feature (~e_i)[cite: 39, 778].
                ...
                # [cite_start]computational graph, enabling second-order gradient computation[cite: 239, 682].
                ...
                # [cite_start]Following Formula (4): I(e_i) ≈ <grad_e_i_L, (e_i - ~e_i)>[cite: 500].
                # [cite_start]This captures the contribution across all non-linear layers to mitigate Layer Bias[cite: 38, 239].
                # [cite_start]'delta_e' represents the distance to the proximal smoothing baseline[cite: 768].
                ...
                # [cite_start]Following Formula (8): L_reg = phi * ||I(E)||_2[cite: 721, 761].
                # [cite_start]This directly sparsifies sensitive feature importance during training[cite: 239, 679].
