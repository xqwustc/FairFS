...
class WideDeep(BaseModel):
    def __init__(self, 
        ...
        self.dnn = MLP_Block(input_dim=embedding_dim*len(feature_map.features),
                             output_dim=1, 
    ...
    def forward(self, inputs):
        ...
        # Wide part: lr_layer
        lr_emb = self.lr_embedding(X)  # (batch, num_fields, 1)
        y_pred = lr_emb.sum(dim=1)
    ...
    def forward_with_embig(self, inputs, inter_type='smooth'):
        ...
        # For FairFS, use the same embedding for both wide and deep parts
        # so gradient flows through self.feature_emb for feature importance calculation
        # Wide part: reshape to (batch, num_fields, emb_dim) -> mean across emb_dim -> sum across fields
        batch_size = self.feature_emb.shape[0]
        total_width = self.feature_emb.shape[1]
        num_fields = total_width // self.embedding_dim
        reshaped_emb = self.feature_emb.view(batch_size, num_fields, self.embedding_dim)
        wide_out = reshaped_emb.mean(dim=2).sum(dim=1, keepdim=True)
        
        y_pred = wide_out + self.dnn(self.feature_emb)
        y_pred = self.output_activation(y_pred)
        return_dict = {"y_pred": y_pred}
        return return_dict
    ...
    def forward_with_fmcr(self, inputs, inter_type = 'zero', seed=2019):
        ...
        y_pred = y_pred.repeat(self.interpolate_n + 1, 1)
